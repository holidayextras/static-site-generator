...
var _metalsmithAssets = _interopRequireDefault(require("metalsmith-assets"));
...
var MetalSmithLoader = function MetalSmithLoader(opts) {
  ...
  var metalSmith = new _metalsmith["default"](opts.src).clean(opts.clean).metadata(opts.config || {}).use(dataSource).use((0, _singleFileOnly["default"])());
  ...
  })).destination(opts.destination).use((0, _metalsmithAssets["default"])({
    source: './' + opts.assets,
    destination: './'
  ...
  if (opts.devMode) {
    metalSmith.use((0, _webpackDevServer["default"])(opts.devMode));
  }
