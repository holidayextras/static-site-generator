  ...
  _createClass(PageData, [{
    ...
    value: function extractData(data, fileName, fileParams) {
      ...
      for (var i = 0; i < data.length; i++) {
        var newFile = extraFiles ? {} : (0, _clone["default"])(this.params.files[fileName]);
        var folder = fileName.split('/');
        folder = folder && folder.length > 1 ? folder.slice(0, -1).join('/') + '/' : ''; // Attach page data to props passed to the page
        ...
        newFile.pagename = newFile.pageName = pageName + '.html'; // Create the new page in metalsmith
    ...
    value: function getDataForPage(res, fileName, fileParams) {
      ...
      return new Promise(function (resolve, reject) {
        ...
        res.on('end', function () {
          try {
            var newData = data.split('').map(function (_char) {
              return _char.charCodeAt(0).toString().length < 4 ? _char : "&#".concat(_char.charCodeAt(0), ";");
    ...
    value: function makeExtraAPICalls(data, fileParams, callBack) {
      ...
      return Object.keys(loop).map(function (opt) {
