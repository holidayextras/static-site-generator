  ...
  return function (files, metalsmith, done) {
    ...
    function retrieveContentForFile(fileName, callback, ctx) {
      ...
      if (file.prismic) {
        ...
        var queryKeys = {};
        var collectionQuery; // create associative array of query keys and their query & formName
        ...
        each(Object.keys(queryKeys), function (currentQueryKey, queriedAndProcessedCallback) {
          ...
          function queryNextPage() {
            ctx.api.form(queryKeys[currentQueryKey].formName).query(queryKeys[currentQueryKey].queryString).pageSize(queryKeys[currentQueryKey].pageSize).page(page).orderings(queryKeys[currentQueryKey].orderings).fetchLinks(queryKeys[currentQueryKey].fetchLinks).ref(ctx.ref).submit(prismicCallback);
          }
          ...
          function prismicCallback(err, d) {
            if (err) {
              ...
              if (queryKeys[currentQueryKey].allPages && page < totalPages) {
                page++;
                queryNextPage();
              } else {
    ...
    function processRetrievedContent(content, filePrismicMetadata, queryKey, ctx) {
      if (content.results != null && content.results.length > 0) {
        ...
        for (var i = 0; i < content.results.length; i++) {
          ...
          result.data = {}; // process the data fragments, invoking helpers to make the data more usable
          ...
          if (content.results[i].fragments != null && Object.keys(content.results[i].fragments).length > 0) {
            for (var fragmentFullName in content.results[i].fragments) {
              ...
              if (content.results[i].data[fragmentFullName].type === 'Range') {
                content.results[i].fragments[fragmentFullName] = {
                  type: 'Number',
                  value: content.results[i].data[fragmentFullName].value
                };
    ...
    function generateCollectionFiles(fileName, collectionQuery, callback, ctx) {
      if (collectionQuery != null) {
        ...
        var fileSuffix = fileExtension != undefined && fileExtension !== "" ? "." + fileExtension : ""; // for every result in the collection query
        ...
        for (var i = 0; i < file.prismic[collectionQuery].results.length; i++) {
          ...
          newFile.prismic[collectionQuery].results = [file.prismic[collectionQuery].results[i]]; // add the filename to the ctx object to make it available for use in the linkResolver function

          ctx.path = fileName; // use the linkResolver to generate the filename removing the leading slash if present
