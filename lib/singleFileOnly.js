  ...
  return function (files, metalsmith, done) {
    ...
    if (process.env.singlePage) {
      Object.keys(files).map(function (fileName) {
        var folderPrefix = opts.webpackOptions.folderPrefix ? "".concat(opts.webpackOptions.folderPrefix.replace(/^\//, ''), "/") : null;
        var pagePrefix = folderPrefix && !process.env.singlePage.startsWith(folderPrefix) ? folderPrefix : '';
