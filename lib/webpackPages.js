...
var webpackPages = function webpackPages(globalOptions) {
  ...
  return function (files, metalsmith, done) {
    ...
    var generateOutput = function generateOutput(template, props, options) {
      var method = props.dataSource && props.dataSource.hydrate ? 'hydrate' : props.dataSource.hydrateRoot ? 'hydrateRoot' : 'render';
      ...
      if (props.store) {
        ...
        if (props.dataSource.hydrateRoot) {
          output += "var renderedElement = ReactDOM.".concat(method, "( ").concat(domNode, ", <Provider store={ store }>").concat(reactNode, "</Provider>);");
        } else {
          output += "var renderedElement = ReactDOM.".concat(method, "( <Provider store={ store }>").concat(reactNode, "</Provider>, ").concat(domNode, ");");
        }
      } else {
        if (props.dataSource.hydrateRoot) {
          output += "var renderedElement = ReactDOM.".concat(method, "( ").concat(domNode, ", <Element {...props} />);");
        } else {
          output += "var renderedElement = ReactDOM.".concat(method, "( ").concat(reactNode, ", ").concat(domNode, ");");
        }
      }
...
var _default = exports.default = webpackPages;