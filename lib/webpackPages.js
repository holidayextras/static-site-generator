...
var webpackPages = function webpackPages(globalOptions) {
  ...
  return function (files, metalsmith, done) {
    ...
    var generateOutput = function generateOutput(template, props, options) {
      ...
      var providers = []; // functions that take children and wrap in each provider
      ...
      if (props.store) {
        ...
        providers = function providers(children) {
          return "<Provider store={ store }>".concat(children, "</Provider>");
        };
      }
      ...
      if (props.compLibEnabled) {
        ...
        providers = function providers(children) {
          return "<ComponentProvider>".concat(children, "</ComponentProvider>");
        };
      }

      var componentString = '<Element {...props} />';
      providers.forEach(function (provider) {
        componentString = provider(componentString);
      });
      output += "var renderedElement = ReactDOM.".concat(method, "(").concat(componentString, ", document.getElementById( 'content' ))");
